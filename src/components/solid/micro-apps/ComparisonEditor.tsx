...
const SupportCellEditor: Component<{ 
  detail: FeatureDetail | null | undefined; 
  isDataDocks?: boolean;
  onUpdate: (newDetail: FeatureDetail) => void;
}> = (props) => {
  ...
  const iconColorClasses = () => {
    ...
    switch (props.detail.level) {
      case "full":
        return "border border-[#4a8136] bg-transparent text-[#4a8136]";
      case "partial":
        return "border border-[#eab308] bg-transparent text-[#eab308]";
      case "none":
  ...
  const cycleLevel = () => {
    if (!props.detail) return;
    const levels: ("standout" | "full" | "partial" | "none")[] = ["standout", "full", "partial", "none"];
    const currentIndex = levels.indexOf(props.detail.level || "none");
    const nextLevel = levels[(currentIndex + 1) % levels.length];
    props.onUpdate({ ...props.detail, level: nextLevel });
  };

  let spanRef: HTMLSpanElement | undefined;

  createEffect(() => {
    // When state changes from external actions (like changing competitor columns),
    // force the DOM to match state so data doesn't accidentally carry over.
    if (spanRef && spanRef.textContent !== (props.detail?.text || "")) {
      spanRef.textContent = props.detail?.text || "";
    }
  });
  ...
  const handleBlur = (e: Event) => {
    ...
    if (!props.detail) return;
    props.onUpdate({ ...props.detail, text: target.innerText });
  };


  return (
    <div class="h-full w-full flex items-center justify-center p-2 rounded-lg transition-all duration-300 group/cell hover:bg-black/5">
      <Show when={props.detail}>
        <div class="flex items-center gap-2 max-w-full text-center relative z-10 font-sans">
          ...
          <button 
             onClick={cycleLevel}
             type="button"
             class="shrink-0 hover:scale-110 transition-transform cursor-pointer outline-none focus:ring-2 focus:ring-blue-500 rounded-full"
             title="Click to change status level"
          >
              {/* Standout (Star) */}
              <Show when={isStandout()}>
                <div class={cn("relative shrink-0 h-5 w-5 rounded-full flex items-center justify-center transition-all duration-300", 
          ...
          <span
            ...
            onBlur={handleBlur}
            class={cn(
              "text-[13px] font-sans leading-tight transition-all flex items-center gap-1",
              "focus:ring-1 focus:ring-blue-300 focus:outline-none rounded outline-none bg-transparent whitespace-pre-wrap",
              isStandout() && props.isDataDocks ? "text-[#fd4f00] font-bold" : "",
              isStandout() && !props.isDataDocks ? "text-[#4a8136] font-bold" : "",
              props.detail?.level === "full" ? "text-[#4a8136]" : "",
              props.detail?.level === "partial" ? "text-[#eab308]" : "",
              props.detail?.level === "none" ? "text-[#9c806d]" : ""
            )}
          >{props.detail?.text || ""}</span>
        </div>
      </Show>
    </div>
  );
...
const FeatureGroupSection: Component<{
  ...
  onToggle: () => void;
  // Editors
  storeDataDocks: Record<string, FeatureDetail>;
  storeCompetitors: Record<string, ComparisonCompetitor>;
  updateDataDocks: (id: string, detail: FeatureDetail) => void;
  updateCompetitor: (compId: string, featureId: string, detail: FeatureDetail) => void;
}> = (props) => {
  return (
        ...
        <div class="divide-y divide-[#ece6de]">
          <For each={props.group.features}>
            {(feature: ComparisonFeature) => {
              const ddDetail = () => props.storeDataDocks[feature.id] || { level: "none", text: "Not available" };
              const comp1Detail = () => props.storeCompetitors[props.competitor1.id]?.features?.[feature.id] || { level: "none", text: "Not available" };
              const comp2Detail = () => props.competitor2 ? (props.storeCompetitors[props.competitor2.id]?.features?.[feature.id] || { level: "none", text: "Not available" }) : null;
              ...
              return (
                <div class={cn(
                  ...
                  {/* DataDocks value */}
                  <div class="flex items-start justify-center border-l border-dashed border-[#ece6de]">
                    <SupportCellEditor 
                        detail={ddDetail()} 
                        isDataDocks={true} 
                        onUpdate={(d) => props.updateDataDocks(feature.id, d)}
                    />
                  </div>
                  ...
                  <div class="flex items-start justify-center border-l border-dashed border-[#ece6de]">
                    <SupportCellEditor 
                        detail={comp1Detail()} 
                        onUpdate={(d) => props.updateCompetitor(props.competitor1.id, feature.id, d)}
                    />
                  </div>
                  ...
                  <Show when={props.competitor2}>
                    <div class="flex items-start justify-center border-l border-dashed border-[#ece6de]">
                      <SupportCellEditor 
                          detail={comp2Detail()} 
                          onUpdate={(d) => {
                             if(props.competitor2) props.updateCompetitor(props.competitor2.id, feature.id, d)
                          }}
                      />
                    </div>
                  </Show>
...
const ComparisonEditor: Component = () => {
  const sortedCompetitors = createMemo(() =>
    [...comparisonCompetitors].sort((a, b) => a.name.localeCompare(b.name))
  );

  // Initialize stores with the existing data
  // We index competitors by ID for easy O(1) updating
  const initialComps: Record<string, ComparisonCompetitor> = {};
  comparisonCompetitors.forEach(c => initialComps[c.id] = JSON.parse(JSON.stringify(c)));
  const defaultDataDocks = JSON.parse(JSON.stringify(datadocksFeatures));

  let savedComps = initialComps;
  let savedDD = defaultDataDocks;
  try {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("comparisonEditorBackup");
      if (stored) {
        const parsed = JSON.parse(stored);
        if (parsed.comparisonCompetitors && parsed.datadocksFeatures) {
          const compRecord: Record<string, ComparisonCompetitor> = {};
          parsed.comparisonCompetitors.forEach((c: any) => compRecord[c.id] = c);
          savedComps = compRecord;
          savedDD = parsed.datadocksFeatures;
        }
      }
    }
  } catch (e) {
  ...
  createEffect(() => {
    const exportData = {
        datadocksFeatures: storeDataDocks,
        comparisonCompetitors: Object.values(storeCompetitors)
    };
    if (typeof window !== "undefined") {
      localStorage.setItem("comparisonEditorBackup", JSON.stringify(exportData));
    }
  });

  const updateDataDocks = (featureId: string, detail: FeatureDetail) => {
      setStoreDataDocks(featureId, detail);
  };
  const updateCompetitor = (compId: string, featureId: string, detail: FeatureDetail) => {
      setStoreCompetitors(compId, "features", featureId, detail);
  };

  const defaultComp1 = comparisonCompetitors.find(c => c.id === "c3-solutions") || sortedCompetitors()[0];
  ...
  // Default expand all for the editor
  const [expandedGroups, setExpandedGroups] = createSignal<Set<string>>(new Set<string>(comparisonFeatureGroups.map(g => g.id)));
  ...
  const resetData = () => {
    if (confirm("Reset to original codebase data? This will clear all your unsaved edits.")) {
      if (typeof window !== "undefined") localStorage.removeItem("comparisonEditorBackup");
  ...
  return (
    <div class="space-y-6">
      ...
      {/* Title & Controls */}
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 text-center md:text-left bg-blue-50 border border-blue-200 p-6 rounded-xl relative">
        <div class="absolute top-0 right-0 bg-blue-500 text-white text-[10px] uppercase font-bold px-3 py-1 rounded-bl-xl rounded-tr-xl">Admin Mode</div>
        <div>
            <h2 class="font-bruta text-3xl uppercase tracking-wide">
            <span class="text-blue-900">GRID EDITOR</span>
            </h2>
            <p class="text-blue-800 text-sm mt-1 max-w-2xl">
                Edit statuses and copy directly in the grid below. The layout matches the public page so you can visualize line wraps accurately. When finished, click Export JSON and share the file with the AI agent.
            </p>
        </div>
        <div class="flex items-center justify-center gap-3 shrink-0 mb-1">
          <button
            onClick={resetData}
            class="text-blue-600 hover:text-blue-800 font-bold py-2 px-4 rounded-lg flex items-center gap-2 transition-colors cursor-pointer text-sm"
          >
              Reset
          </button>
          <button
            onClick={exportJSON}
            class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg shadow-sm flex items-center gap-2 transition-colors cursor-pointer"
          >
          ...
          <div class="flex flex-col relative w-full h-full group">
            ...
            <div class="absolute -top-1.5 -right-1.5 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
               <button
                  ...
                  class="h-5 w-5 rounded-full bg-white border border-[#ece6de] flex items-center justify-center text-[#ad9686] hover:bg-[#ece6de] hover:text-[#5f483a] transition-colors shadow-sm cursor-pointer"
                  title="Remove column"
               >
        ...
        <For each={comparisonFeatureGroups}>
          {(group) => (
            <FeatureGroupSection
              ...
              storeDataDocks={storeDataDocks}
              storeCompetitors={storeCompetitors}
              updateDataDocks={updateDataDocks}
