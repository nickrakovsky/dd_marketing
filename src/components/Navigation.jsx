...
const Navigation = ({ isOrange = false }) => {
  ...
  return (
    <nav className={`z-50 transition-colors duration-300 ${isOrange ? "relative bg-transparent" : "relative bg-[#FFF8E9] dark:bg-gray-900"
      }`}>
      <div className="container mx-auto px-0 md:px-8 pt-4 md:pt-12 pb-4 md:pb-6">
        <div className="flex items-center justify-between flex-wrap min-h-[4rem] gap-2 sm:gap-4 px-4 md:px-0 py-2 md:py-0 w-full">
          ...
          <div className="flex items-center flex-shrink min-w-0">
            <a href="/" className="flex-shrink min-w-0">
              <img
                ...
                width="201"
                height="43"
                className={`h-[clamp(1.5rem,10vw,2.5rem)] sm:h-12 w-auto max-w-[65vw] object-contain ${isOrange ? "hidden" : "block dark:hidden"
                  }`}
              />
              <img
                ...
                width="201"
                height="43"
                className={`h-[clamp(1.5rem,10vw,2.5rem)] sm:h-12 w-auto max-w-[65vw] object-contain ${isOrange ? "block" : "hidden dark:block"
                  }`}
              />
            </a>
          </div>
          ...
          <div className="hidden lg:flex items-center lg:space-x-6 xl:space-x-8 flex-1 justify-end">
            ...
            >
              <button className={`flex items-center gap-1 transition-colors font-recoleta text-base xl:text-lg ${isOrange
            ...
            <Button
              asChild
              variant="default"
              className={`px-6 font-recoleta rounded-none text-base xl:text-lg border-b-4 transition-all shadow-none ${isOrange
          ...
          <button
            ...
            className={`lg:hidden shrink-0 p-2 transition-colors duration-300 ${isOrange ? "text-white" : "text-foreground dark:text-gray-200"
              }`}
        ...
        {isMobileMenuOpen && (
          <div className={`lg:hidden absolute top-full left-0 right-0 border-t z-50 shadow-xl transition-colors duration-300 ${isOrange
            ...
            <div className="flex flex-col space-y-0">
              <div className="flex flex-col">
                <button
                  ...
                  className={`flex items-center justify-between px-4 py-3 font-recoleta border-b transition-colors duration-300 ${isOrange
                  ...
                  <div className="bg-[#EFE2D2] dark:bg-gray-800 flex flex-col transition-colors duration-300">
                    {dropdownItems.map((item) => (
                      <a
                        key={item.href}
                        href={item.href}
                        className={`px-8 py-3 font-recoleta border-b last:border-0 transition-colors duration-300 ${isOrange
              ...
              <div className="p-0">
                <Button
                  asChild
                  variant="default"
                  className={`w-full rounded-none border-b-4 transition-all shadow-none font-recoleta py-8 ${isOrange
                      ? "bg-white text-[#fd4f00] hover:bg-black hover:text-white border-transparent hover:border-b-white"
                      : "bg-[#FF5722] hover:bg-black text-white border-transparent hover:border-b-[#FF5722]"
                    }`}
                >
