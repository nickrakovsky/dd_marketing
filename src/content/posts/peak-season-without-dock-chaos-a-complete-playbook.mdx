...
relatedFeatures: ['capacity-limits']
...
<RelatedFeatureCallout id="capacity-limits" />
...
*   A live [dock dashboard](/datadocks-features/dock-dashboard) that shows every appointment, door assignment, and load status in real time.
*   [Capacity limits](/datadocks-features/capacity-limits) that prevent overbooking and enforce peak season rules automatically.
*   A [carrier portal](/datadocks-features/carrier-portal) that gives carriers 24/7 access to book and manage appointments without calling the facility.
*   [Live editing](/datadocks-features/dock-dashboard) so coordinators can adjust the schedule on the fly when carriers run late or emergencies arise, with automatic notifications to everyone affected.
...
For tools to track these metrics in real time, see [efficiency reports that track peak performance](/datadocks-features/efficiency-reports).
